...
function inKorea(lat: number, lng: number): { lat: number; lng: number } | undefined {
...
interface NaverItem {
  ...
  address?: string;
  roadAddress?: string;
  /** WGS84 longitude × 10⁷, as a string. */
  mapx?: string;
  /** WGS84 latitude × 10⁷, as a string. */
  mapy?: string;
}
...
export function parseNaver(json: NaverResponse): PoiPlace[] {
  ...
  return items.map((it) => {
    ...
    return {
      ...
      ...inKorea(Number(it.mapy) / 1e7, Number(it.mapx) / 1e7),
    };
  });
}
...
export function parseFoursquare(json: FsqResponse): PoiPlace[] {
  ...
  return results.map((r) => {
    ...
    return {
      ...
      ...inKorea(Number(r.latitude), Number(r.longitude)),
    };
  });
}
