/**
 * The Seoul subway graph and its route planner — pure, so the traveller's
 * phone can run it too.
 *
 * The server plans routes between named stations with this. The phone plans the
 * route from wherever the traveller is standing with the very same code and the
 * same snapshot, because that route begins at their position, and their
 * position does not leave the phone (src/lib/deviceTask.ts). Refreshing the
 * list from Seoul Open Data is the server's job (sources/subwayGraph.ts).
 *
...
export function fromSnapshot(): SubwayGraph {
...
export function planRoute(graph: SubwayGraph, fromName: string, toName: string): SubwayRoute | undefined {
  return planBetween(graph, findStationCodes(graph, fromName), findStationCodes(graph, toName));
}

/**
 * Every code a station goes by. Seoul Station is "서울역" on four lines and
 * "서울" on GTX-A, so the name alone finds only some of its platforms.
 */
export function stationCodes(graph: SubwayGraph, ko: string): string[] {
  const bare = ko.replace(/역$/, "");
  return [...new Set([...findStationCodes(graph, bare), ...findStationCodes(graph, `${bare}역`)])];
}

/** The same, between sets of station codes — for a phone that knows its stations by position, not by name. */
export function planBetween(graph: SubwayGraph, startCodes: string[], goalCodes: string[]): SubwayRoute | undefined {
  const starts = startCodes;
  const goals = new Set(goalCodes);
