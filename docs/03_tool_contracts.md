...
- **output(Markdown)**: 장소 3~6개, 각 항목 한 줄 요약+친화 배지 + **🗺️ Naver/Kakao 맵링크**(D-032, 장소별). "temple stay"는 큐레이션 프라이머 리드(D-030). 끝에 맥락 팔로업 칩(D-031). (이미지는 N11로 제거, 맵링크 URL은 유지.)
...
- **output**: 가능/불가 방법 + 대안(현금/충전 등) 영문 정리. 끝에 **상황별 브릿지 칩**(D-017, N4식): ATM→Global ATM 찾기, 병원→약국·응급, 식당→메뉴, 교통/택시→경로 등 형제 툴 연결.
- **확장(D-016)**: 상황 매트릭스에 **KTX/SRT(국내카드전용 키오스크), 세금환급(VAT), ATM(Global ATM·DCC·4자리PIN), 온라인/앱결제(본인인증 차단), 병원/응급(F1)** 추가 + 택시 안티스캠(미터·1330)·T-money 심화(태그아웃·환승·환불·Climate Card/K-Pass).
...
- **description(영문)**: 한국 본인인증/현지인 가정 서비스 우회 — 택시앱·배달·예약·온라인결제·**콘서트/이벤트 티켓팅(D-018)**·카톡가입·SIM·**은행/송금(D-020, ARC벽→Wise/Revolut)**·세금환급·입국서류·응급·키오스크. `Part of Korea Trip Concierge(코리아 트립 컨시어지).` (12서비스+GENERIC)
...
- **데이터**: **큐레이션 동네가이드**(키 불필요·항상 동작). **35개 동네**(D-014로 8→21, D-017로 21→26, D-018로 26→29, D-020로 29→32, D-021로 32→35): 서울 23(명동·홍대·강남·인사동·성수·이태원·북촌·동대문·여의도·잠실/롯데월드·익선동·을지로·삼청동·가로수길/신사·연남·압구정/청담·종로/광화문·합정/망원·서촌·건대·신촌/이대·해방촌/경리단·대학로), **부산 5**, **제주 2**, **강원 1**(강릉), **전북 1**(전주), **경북 1**(경주), **인천 1**(차이나타운/송도), **강원+1**(속초). `keys` 정규식에 한글 별칭 포함, 첫 매칭 우선(순서 주의). *searchPlaceForeigner는 일반 sightseeing query에 다도시 must-see 시딩(D-021, Seoul/Busan/Jeju/Gyeongju).*
- **output**: 동네 요약 + 스팟 3~5 + 이동 팁 + **🗺️ Naver/Kakao 맵링크**(D-032) + interest별 노트. 끝에 **맥락 팔로업 칩**(D-031: 동네명 담은 "{area} 가는 길/유명가게/페르소나 코스/지금" → 다른 툴 연결). 미수록 동네는 "실제 검색" 유도 폴백.
...
- **output**: 지금 상태(판정 헤드라인 + 영업시간 + 현재 KST 시각 + 닫는요일 + 한줄노트 + **🗺️ Naver/Kakao 맵링크**(D-032) + 실시간 날씨·미세먼지) + 추천 여부. 끝에 선택지(대안 시간/대안 장소). ⚠️ "crowd level"은 미구현(데이터원 없음) → 설명에서 제외(R-DOC 정합).
- annotations: readOnly true / idempotent false / openWorld true
