...
## 7. 문서 맵
`docs/00`(오버뷰) `01`(카카오규칙·절대) `02`(제품스펙) `03`(툴계약) `04`(UX칩) `05`(작업협약) `06`(결정로그) `07`(진행SSOT) `08`(키발급가이드) `09`(대화예시3) `10`(이전 핸드오프) `11`(배포런북) `12`(로컬API신청가이드) `13`(이 문서). 코드: `src/server.ts`(전송) `src/tools/*` `src/lib/*` `src/lib/sources/*`.
