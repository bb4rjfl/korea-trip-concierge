...
└─────────────┘   답변+선택지칩    │  본선: Kakao Tools) │
                          ...
                          ┌────────────────────────────────┐
                          │  우리 MCP 서버                   │  ← 우리가 만든 것
                          │  korea-trip-concierge (KC 배포)  │     (10개 도구)
                          └─────────┬──────────────────────┘
                                    │ ③ 필요 시 외부 데이터 조회
                                    ▼
                          ┌────────────────────────────────┐
...
→ 우리가 만드는 것 = **"카카오톡 속 LLM이 부를 수 있는, 한국 여행 전문 도구 10개".**
...
// 응답: 10개 도구 각각 { name, description, inputSchema, annotations }
```
...
## 참고 (현재 구현 상태)
- **10개 도구** 전부 계약·코드 완성. 지식툴 3종(C-5/6/7)은 키 없이 작동, API툴 7종은 실데이터 검증 완료(서울 버스/지하철·카카오 Local은 검토 중).
- 코드 위치: `src/server.ts`(전송), `src/tools/*`(도구 10), `src/lib/*`(공통: 칩·24k가드·캐시·타임아웃·네이밍린트), `src/lib/sources/*`(TourAPI·TAGO·ODsay·VisitJeju·기상청+에어코리아).
- 검증: `npm run build` / `npm test`(70 green) / `scripts/verify-live.ts`(실 API 호출 점검).
