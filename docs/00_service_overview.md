...
- **해결하는 막힘 4종**: 🗺️ 장소 탐색 · 🏪 외국인 친화 매장 · 🚌 대중교통 · 💳 결제
- **핵심**: T머니·환승·해외카드 가능 매장·실시간 버스처럼 **외국인이 검색만으로 못 푸는 "한국 특수 맥락"** 을 영어로 정리.
...
└─────────────┘   답변+선택지칩    │  본선: Kakao Tools) │
                          ...
                          ┌────────────────────────────────┐
                          │  우리 MCP 서버                   │  ← 우리가 만든 것
                          │  korea-trip-concierge (KC 배포)  │     (8개 도구)
                          └─────────┬──────────────────────┘
                                    │ ③ 필요 시 외부 데이터 조회
                                    ▼
                          ┌────────────────────────────────┐
...
→ 우리가 만드는 것 = **"카카오톡 속 LLM이 부를 수 있는, 한국 여행 전문 도구 8개".**
...
| 실시간 버스 | trackBusArrival | 국토부 TAGO | ⏳ 키 발급 후 |
| 관광/매장/영업시간 | searchPlaceForeigner, findForeignerFriendlyStore, getNowInfo | 한국관광공사 TourAPI(영문) | ⏳ 키 발급 후 |
| 경로 | getTransitRoute | ODsay | ⏳ 키 발급 후 |
...
// 응답: 8개 도구 각각 { name, description, inputSchema, annotations }
```
...
- **트리거**: "Bus 143, how many stops to Myeongdong Cathedral?"
- **입력**: `busNumber`(필수), `dropOffStop`(필수), `currentStop?`
- **처리**: **TAGO** 2단계 — ① 정류소명→`getSttnNoList`로 nodeId+cityCode 해석 ② `getSttnAcctoArvlPrearngeInfoList`로 그 정류소 도착정보 조회 → 노선번호로 필터 → **남은 정거장수 + 예상 분**. 실시간이라 캐시 10초.
...
- **입력**: `place`(필수)
- **처리**: TourAPI `searchKeyword2`로 장소 특정 → `detailIntro2`로 영업시간/휴무 → **현재 한국 시각(KST)** 과 대조해 안내. (날씨/혼잡은 향후 KMA 연동 예정 — 현재는 시간 기반.)
- **출력**: 현재 KST + 영업시간/휴무 + 늦은 시간 경고. 과도한 단정 없이 정직하게.
...
## 참고 (현재 구현 상태)
- 8개 도구 전부 **계약·코드 완성**. 지식툴 3종(C-5/6/7)은 키 없이 작동, API툴 5종은 키 발급 후 실데이터로 전환.
- 코드 위치: `src/server.ts`(전송), `src/tools/*`(도구), `src/lib/*`(공통: 칩·24k가드·캐시·타임아웃·네이밍린트), `src/lib/sources/*`(TourAPI·TAGO·ODsay).
- 진행 상황은 `07_progress.md`(SSOT), 키 발급은 `08_api_key_issuance.md`, 데모 예시는 `09_demo_conversations.md`.
