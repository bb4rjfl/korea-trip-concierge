...
## 마일스톤
- [x] **여행자 프로파일 추천 (2026-09-02, D-044)**: `profile.ts` 신설 — 대화에서 예산·페이스·보행·아동·식이·거부테마를 4개 국어로 읽어 **코스를 실제로 필터·리페이싱**(`_Planned for: …_` 한 줄로 읽은 내용 고지). 시그니처 코스는 프로파일이 있으면 우회, **'another'는 이전 variant 픽을 소진해 구조적으로 중복 불가**, 오케스트레이터가 최근 발화를 `notes`로 전달, **비 오면 실내로 자동 재구성 + 이유 고지**. 풀 위생(의료업소 제외·요약 잘림·가짜 환승·같은 랜드마크 중복) + `\b` 이중 이스케이프 소스 감사 테스트(`PRICEY`가 실제로 무력화돼 있었음). **412 tests.**
- [x] **추천 엔진 개편 + 도메인 승격 (2026-08-31, D-043)**: 라이브 후보 풀(`livePool.ts`, 서울 522개) + variant 변주로 "another"가 실제 동작, 몰 큐레이션(`malls.ts`), **https://ktc.tacita.cloud** 정식 주소 + OG 카드, `npm run audit:facts` 요금 실측 감사. **390 tests, 배포·검증.**
- [x] **본선 품질 라운드② (2026-08-31, D-042)**: 도구별 스트레스 스윕 + **요금 실측 감사**(지하철 기본요금 ₩1,400→₩1,550, 신분당선 부가운임 ₩700) + 실전 니즈 4종(짐보관·빨래·기도실·EMS) + **4개 국어 배관 구멍**(검색어 정리기·도시 좌표 앵커가 영/한만) + 한국어 로마자 병기 제거 + LLM-off 스윕. **384 tests, 프로덕션 배포·검증.**
- [x] **도시간 이동 실데이터 + 해치 준비 (2026-08-31, D-041)**: data.go.kr 4종 승인 → `intercityApi.ts`로 KTX·시외버스 **실제 시각·요금**(서울→경주 KTX 20:58 ₩49,300). 서울 상징 캐릭터 해치는 공공누리 4유형 확인 후 **승인 전 노출 불가 스위치**로 자리만 확보하고 신청서류 작성(`submission-tourism/haechi/`). **371 tests, 프로덕션 배포·검증.**
- [x] **본선급 품질 라운드 (2026-08-31, D-040)**: 실제 관광객 시나리오 스윕 → 결함 일괄 수정. 역명 이중표기·경로 옵션(빠른/쉬운/싼)·공항철도 요금·도시별 이동안내·에티켓·무장애·시즌카드·**TourAPI 축제 신규 연동**·ja/zh 음식 의도·프랜차이즈 강등·`_라벨_` 미번역·**2단계 스트리밍(9.1s→2.5s)**·**PWA 오프라인**·레이트리밋. **369 tests, 프로덕션 배포·검증 완료**(https://180-210-78-148.sslip.io).
- [x] **관광공모전 웹앱 트랙 — 서울 교통 자립 + QA 결함 일괄 수정 (2026-08-31, D-039)**: 서울버스 직통 플래너(TOPIS 3체인·다중 후보 정류장 교차) + 지하철 그래프 위상 정정(공항철도/1·2·3·4호선/수인분당·경의·서해선 재구성 + 노선별 표정속도) + 공항철도 실요금. 부수 결함: 몇 번 출구(`src/lib/exits.ts` 36곳, 경로·장소·단독 답변), 요일별 영업시간 판정(토요일에 평일 마감시간 오표시), 내일 예보, 알레르기 동의어(shrimp/エビ/새우→shellfish), 카드 라벨 결정적 번역(`web/server/labels.ts`), 번체 중국어 응답, 동일답변 반복 고지. **소스 위생 테스트 신설**(히어독이 먹은 `\b`가 백스페이스로 박혀 정규식이 조용히 무력화된 버그 2회 재발 방지 — 응급실 `\ber\b` 포함). **364 tests green, 프로덕션 배포·검증 완료**(`03f4696`, https://180-210-78-148.sslip.io).
...
src/lib/sources/         TourAPI, TAGO(전국버스), ODsay(경로), VisitJeju, weatherair(기상청+에어코리아), seoulSubway(지하철), visitseoul, poi, **seoul(서울버스 TOPIS, D-026)**
...
src/lib/courses.ts       페르소나 코스 데이터/엔진(D-025/D-029) — 태깅 스팟 서울35·부산15·제주15·경주11(총76) + 10페르소나 테마맵(beauty/family/couple/kpop/foodie/culture/nightlife/nature/solo/budget)+GENERIC + 도시인식 조합엔진(반나절/1일/2일/3일·존클러스터·스왑대안, Seoul/Busan/Jeju/Gyeongju) + 1-day 시그니처 골든코스 6. 순수·결정적·테스트락.
...
test/                    vitest 247개 (헬퍼 + 로마자 + 퍼지 + 랜드마크 + 다국어 + 폴리시 v4 + 콘텐츠 + 지오코딩 + 다도시 must-see + 완성도 + v5/v6수정 + 페르소나코스/조합엔진 + 서울버스 + 전체 툴 계약 + 핸들러 스모크 + 소스 파서)
Dockerfile               linux/amd64, 루트
```

## 💰 수익화/BM 로드맵 + 규칙 분석 (2026-06-29, 유실 방지)
> 사용자 BM 아이디어(업체등록·예약·후기·광고·미용/사진관/병원 연결로 수익화) 검토 결과. **결론: BM 자체는 길이 있으나 "공모전 제출 MCP 응답 안"이 아니라 "그 바깥(별도 상업 앱/웹 또는 포스트-콘테스트)"에서.**
- **MCP 응답 내 직접 BM = PlayMCP 규칙 위반(반려)**: docs/01 §3-5 광고 노출 유도 금지 · §4 상업링크·구매유도·리워드 "과다" 반려 · 도구실행에 유료결제/구독 필수 반려 · 개인정보 6종(여권/카드/계좌) 수집·전송 반려(=MCP 안 결제·여권수집 불가).
- **허용 범위**: 아웃링크는 OK(과하지 않게) — 공식 예약페이지·후기·상담채널로 담백한 연결은 가능. 선=광고/커미션/구매유도/랭킹조작이면 위반, 유용한 공식정보 연결이면 OK.
- **의료(피부과/성형) 별도 — 의료법**: 환자 **유인·알선(소개비/커미션 병원연결) 금지**(법 위반, PlayMCP 이전 법적 리스크). 상담채널 안내도 '유인·알선' 되면 위험. 커미션 소개 ❌, 병원 자체 공식 상담채널 정보 안내(커미션X·특정병원 푸시X)는 회색~안전. **정확한 선은 법률검토 필요.**
- **권장 경로**: ①공모전 단계=MCP 순수 큐레이션 유지(대상 노리는 데 유리) — 페르소나 코스(D-025)가 그 일환. ②BM=별도 트랙(예약/후기/광고/커미션은 별도 앱·웹, 의료는 의료법 준수). MCP는 funnel/쇼케이스.

## 세션 로그
- 2026-07-03 (종합검토 + 운영 하드닝 + 코스 길찾기, D-038) **최종 심사 전 종합검토 → 하드닝**. 사용자 요청으로 서비스 전반 검토(목적적합성·가치·심사기준별·1등경쟁력·리스크) + 구조/데이터플로우 도식화(show_widget). 최우선 리스크=**ODsay egress-IP 의존·경로 콜드스타트** → **하드닝**: `directionsLinks`(카카오맵 sName/eName 이름기반 길찾기+네이버맵)를 getTransitRoute **전 경로(성공/미연결/지오코딩실패/무경로/타임아웃)**에 부착(실패해도 사용자가 지도앱으로 이동 가능) + **`/egress-ip` 진단**(재시작 후 ODsay 재등록 판단). **코스 원클릭 길찾기**(route칩이 첫 2스톱명). 서울스팟 +3. **커밋 e9613f2, 270 tests green.** ✅ 배포·검증: 라이브 `build:e9613f2`, `/egress-ip`=210.109.82.101(ODsay 등록IP 일치), 코스 route칩 스톱명 라이브 확인. **최종 시나리오 테스트(30, 프로덕션 경로 e9613f2) 완료 = 🟢 GO**: 🔴 0, 27 PASS + 3 ⚠️(전부 graceful degrade). ⚠️①②=ODsay 라이브 콜드 타임아웃(D-038 directions 폴백이 그대로 커버, 재테스트 시 정상) ⚠️③=getNowInfo 가비지입력이 clean not-found 대신 live-lookup 타임아웃 분기. 안전 클린(인젝션 무시·XSS 무반사·-32602 무누출·PII안전), 신기능 전부 라이브(맥락칩·맵링크·도보/템플스테이 프라이머·신페르소나·신랜드마크·육회). **후속 하드닝**: ⚠️③ **`isPlausiblePlace` 가드**(getNowInfo — 마크업/제어문자/무letter 입력은 네트워크콜 전에 clean not-found + 에코 sanitize) +2 tests → **272 green**. ⚠️①②=ODsay 라이브 재검증 2/2 성공(Hongik→Gangnam·Incheon공항→Myeongdong 실경로 반환) → 콜드윈도 transient 확정, 폴백 존재로 무조치(2.5s×2 retry는 p99<3s와 트레이드오프라 유지). **종합판정: 우승권 출품작, 예선 제출 손색없음. 1등 레버=①본선 Widget ②데모스토리 ③운영하드닝(완료).** ⏳ ⚠️③ 가드 재배포 후 라이브 확인.
- 2026-06-30 (도보해설관광 + 칩 라이브검증 + 콜드스타트, D-033~D-036) **카카오 AI채팅 실측으로 칩/링크 검증·강화**. 사용자가 카카오톡 스크린샷(getAreaGuide 칩 누락) 공유 → **playmcp.kakao.com AI채팅(강상호 로그인, 컴퓨터유즈/Chrome)로 직접 실측**: **D-033** 호스트 LLM이 응답을 paraphrase하며 "Tap to continue" 칩 누락 입증 → footer에 **조합 LLM 명시 지시문** 추가 → **칩 라이브 생존 확인**(getAreaGuide·recommendTripCourse·searchPlaceForeigner 전부). **D-032** 맵링크 카카오 우선, 라이브 클릭링크 확인. **D-034** 서울 도보해설관광(공식 무료 다국어) 큐레이션 프라이머 + Seoul 코스 🚶칩(게이팅 수정=음식스톱에 안 가려지게). **D-035** searchPlaceForeigner 결과칩 맥락화(getAreaGuide와 통일, "How do I get to {area}?"·도시가드) — Insadong 라이브 확인. **D-036** 라이브에서 **재배포 직후 첫 POI쿼리 콜드스타트 타임아웃** 발견 → `warmup.ts` 부팅 커넥션 예열(8 origin). 부수발견: **카카오맵 MCP와 도구함 경쟁**(외국인쿼리=우리/일반=카카오맵, 올바른 특화+대회는 단독제출이라 무관). **커밋 c47d910/07f896b/a9a3fe6/9eb472a/8132b75/d57958e/(D-036), 263 tests green.** ⏳ D-036 재배포 후 첫쿼리 즉시성공 확인.
- 2026-06-29 (이어서, 칩 UX + 맵링크, D-031/D-032) **호스트 재작성 견디는 팔로업 칩 + Naver/Kakao 맵링크**. 사용자가 카카오 AI채팅 스크린샷 공유: getAreaGuide 응답이 **호스트 LLM에 paraphrase**되며 "Tap to continue" 칩이 누락됨(큐레이션 blurb가 다른 문장으로). **D-031**: `footer.ts` 칩을 자연어 팔로업 질문으로 재구성(💬 You can ask me next, 백틱/메타 제거), getAreaGuide 칩을 동네명 담은 맥락 질문으로(가는길·유명가게·페르소나코스·지금). 테스트 6곳 갱신. **D-032**: `maplinks.ts` Naver+Kakao 맵 딥링크 → searchPlaceForeigner(장소별)·getAreaGuide(동네)·getNowInfo(장소). **커밋 c47d910(칩)·07f896b(맵링크), 256 tests green.** ✅ **배포·검증**: 라이브 `build:c47d910`에서 서브에이전트 10/10 PASS(맥락칩 확인·서울버스 2모드·경주·solo+budget·템플스테이·안전/인젝션 클린·회귀 intact). ⚠️ **칩이 실제 카카오 재작성에서 surface되는지는 사용자 앱 재테스트로 최종확인**(raw 출력엔 정상). 맵링크는 07f896b 배포 후 라이브 확인 예정.
- 2026-06-29 (이어서, A+B+C+TourAPI, D-027~D-030) **서울버스 여정통합 + 코스 Phase3 + 템플스테이 + TourAPI 영향평가**. 사용자 "a,b,c 전부 + TourAPI 변경공지 확인 후 필요작업 + 시나리오 테스트". **D-027 TourAPI**: data.go.kr 법정동 변경공지(인천/광주전남통합, 7/1 시행) 측정→`tourapi.ts`는 키워드/좌표/contentId만 써 areaCode/법정동 미사용=**영향 없음, 코드 무변경**(grep 전수 확인). **D-028 A(서울버스↔여정통합)**: getTransitRoute 버스칩에 하차정류소 적재(route→track 원탭), trackBusArrival dropOffStop optional+서울 **노선위치 모드**(getBusPosByRtid, 지하철 line 모드 대칭), `seoul.ts` getSeoulBusPositions. **D-029 B(코스 Phase3)**: 경주 4번째 코스도시(11스팟)+부산+5/제주+4 추가(멀티데이 밀도, 총 스팟 76)+4페르소나(총 10: nightlife/nature/solo/budget). **D-030 C(P3)**: searchPlaceForeigner 템플스테이 큐레이션 프라이머(공식 eng.templestay.com+외국인친화 사찰). P4·Jamsil은 D-020에서 이미 처리 확인. **커밋 102e526(A)·a49da54(B)·21c1953(C), +7 tests(247→254 green), 빌드 클린.** ⏳ KC 재배포(중지→시작) 후 라이브 검증 + 시나리오 테스트 예정(사용자 허용요청 대기).
- 2026-06-29 (이어서, 새 세션) ✅ **서울버스 KC 배포·검증 완료 + Phase2 라이브 확인**: 핸드오프(docs/25)는 "7fd81c2 미배포"였으나 **라이브 헬스 = `build:7fd81c2`, 13툴, sources 전부 true(bus:true)** = 이미 배포됨. **KC 프로덕션 경로(연결된 MCP `koreatrip-trackBusArrival`)로 서울버스 실검증**: `{Seoul, 143, 신사역}` → "🛑 Arriving now — get ready to get off!" **실시간 동작**(arrmsg1 파싱·하차 카운트). **로컬 IP throttle(빈 응답)과 달리 KC egress IP는 정상** = 몇 주 블로커 완전 종결, end-to-end 검증됨. **recommendTripCourse Phase2도 라이브 확인**(Busan 2-day, "20s woman, foodie" 조합 페르소나 테마 블렌딩 정상 — 도구함 캐시 설명은 "Seoul/2-day" 구판이나 실서버는 풀엔진). **서울버스 키 미재잠금**(getStationByName stSrch=강남 headerCd 0 재탐침). 코드/테스트 무변경(247 green). → docs 03/07·CLAUDE의 D-025/D-026 "재배포 대기"를 "배포·검증 완료"로 갱신.
- 2026-06-29 🚌 **서울버스 키 드디어 해금!**(매세션 재탐침 끝): `ws.bus.go.kr` headerCd=**0** "정상처리"(BUS_API_KEY 그대로, 몇 주 error30 전파지연 해소). **API 체인 전수 측정 완료**(05시에도 실시간 동작): `busRouteInfo/getBusRouteList`(strSrch→busRouteId, 143=100100022) · `busRouteInfo/getStaionByRoute`(busRouteId→정류소 113개 순서: seq·stationNm·arsId·station[stId]) · `buspos/getBusPosByRtid`(busRouteId→버스위치 sectOrd[구간순서]·lastStnId·vehId·plainNo) · `stationinfo/getStationByName`(stSrch→stId·arsId·stNm) · `arrive/getLowArrInfoByStId`(stId→**arrmsg1 "15분11초후[8번째 전]"**·rtNm). 응답=XML. **→ 즉시 `src/lib/sources/seoul.ts` 구현 완료(D-026)** + trackBusArrival 서울분기(route→stops→getLowArrInfoByStId arrmsg1 파싱, route/stops 1h 캐시, **빈-itemList throttle 3회 재시도**). +7 tests(247 green), 빌드OK. ⚠️ **로컬 IP는 반복호출로 throttle(빈 응답)** → 로컬 e2e 막힘, **KC egress IP(별개)로 배포 후 검증 필요**. 키 재잠금 위험에 빠르게 배포.
- 2026-06-29 (신툴 페르소나 코스 + BM 분석, D-025): 사용자 아이디어("외국인 페르소나별 인기 코스 추천 UX" + 예약/광고 BM)에서 **㉮ 신툴 `recommendTripCourse`(13번째)** 구현 — 20대여성(K-뷰티/사진관/한복)·가족·커플·K-pop팬·푸디·문화/역사·첫방문 7페르소나, 각 코스가 기존 툴로 칩 연결. 순수 큐레이션(규칙안전), 시술/의료는 정보수준만(의료법). **㉯ BM 컴플라이언스 결론**=위 "수익화/BM 로드맵" 섹션 기록. **이어 대폭 강화(사용자 요청)**: `src/lib/courses.ts` 신설 — **조합 가능 페르소나**("20s woman, foodie")·**기간**(반나절/1일/2일)·테마·위치 입력 → 태깅 서울스팟 35 + 7페르소나 테마맵 기반 **조합엔진**(존클러스터 동선·시간블록·스왑대안) + **1-day 시그니처 골든코스 5**(하이브리드). **Phase 2도 완료**(부산10·제주11 스팟 + 도시인식 엔진[Seoul/Busan/Jeju] + **3-day**[4+→3day] + couple 시그니처; 3대도시 외=대구 등만 graceful steer). **+13 tests(240 green)**, 13툴 빌드OK. ⏳ 재배포 대기. (라이브 검증: 강화판 build 1f91386 코스출력 풍부 확인 완료.)
