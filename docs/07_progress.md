...
- [ ] **Phase 1**: getTransitRoute에 모드 라벨(🚇/🚌) + **동적 추적 칩**(탭→해당 버스/역 추적 직결). 새 툴·데이터 0.
- [ ] **Phase 2**: 지하철 **하차 안내**(목적지까지 몇 정거장·지금 어디) — 노선별 역순서 데이터 필요(주요 노선부터). trackBusArrival은 이미 "하차까지 몇 정거장" 구현됨.
- [ ] **구조·서비스 전체 점검** (툴 경계/중복/일관성)
- [ ] **`/check`** 카카오 규칙 최종 점검
- [ ] **콘텐츠 보강 + 로마자 사전** (부대찌개 등 메뉴/지역/결제 + 서울대입구·지선 등 로마자)
- [ ] **전체 UI/UX 플로우 확인**
- [ ] **시나리오 ~100개** 생성 → 전체 흐름 편의성 점검(끊김/막힘 발견)
