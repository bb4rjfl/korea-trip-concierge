...
src/tools/               12개 툴 (types, index, *.ts) — 지식툴4 즉시동작 + API툴8 실연동. getNowInfo=랜드마크 오버레이→VisitSeoul→TourAPI 폴백, getAreaGuide=26 동네, landmarks ~36 명소(D-017)
...
test/                    vitest 198개 (헬퍼 + 로마자 + 퍼지 + 랜드마크 + 다국어 + 폴리시 v4 + 전체 툴 계약 + 핸들러 스모크 + 소스 파서)
