...
- [x] **웹 챗 품질 = 숫자로 관리(2026-09-08 기준, D-048~D-052)**: 하이브리드 검색(BM25+임베딩·RRF, CJK 바이그램)→질의이해→**리랭커**(순서만 되돌려줌, lite·1.5s·실패 시 융합 순서 유지)→도구 실행→**작문 레이어**(사실에 없는 주장·빠뜨린 핵심을 기계적으로 검사해 위반이면 카드로 폴백). 하네스 2종: **`npm run eval`**(대화 17시나리오, 골든셋+심판+결정론 가드+칩 따라가기) / **`npm run eval:rank`**(랭킹 32케이스, 리랭킹만 켜고 끄는 A/B). 현재 **프로덕션 93%**, 랭킹 top-1 94%·MRR 96%, **511 tests**, `de60d18` 배포(https://ktc.tacita.cloud). ⚠️ **대화 하네스는 ±7포인트로 흔들린다** — 이 크기 이하의 변화는 랭킹 하네스나 결정론 가드로 재고, 점수만 보고 채택/기각하지 말 것. ⚠️ **작문 레이어는 표현을 다시 쓴다** — 우리 렌더링의 문구를 정규식으로 읽어 무언가를 판단하는 코드는 조용히 죽는다(제목·`Planned for`·코스 카운트로 세 번 겪음). 형태(시간 블록·굵은 이름)로 판정하거나 복원할 것.
