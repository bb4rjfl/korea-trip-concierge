...
- [~] **신툴 페르소나 코스 + BM 분석(2026-06-29, D-025)**: **`recommendTripCourse`(13번째 툴)** — 7페르소나(20대여성 K-뷰티/사진관·가족·커플·K-pop·푸디·문화·첫방문) 인기코스 큐레이션→기존 툴 칩연결. 순수 큐레이션(규칙안전), 시술/의료는 정보수준만(의료법). **232 tests green.** **BM(광고·예약커미션) 분석**: MCP 응답 내 직접 BM=규칙위반(§3-5/§4)+의료법, BM은 별도 트랙(docs/07 수익화 로드맵). ⏳ 재배포 대기.
