...
- [x] **A+B+C+TourAPI 완료(2026-06-29, D-027~D-030, 커밋 102e526/a49da54/21c1953)**: **A** 서울버스↔여정 통합(getTransitRoute 버스칩 하차정류소 적재→trackBusArrival 원탭, dropOffStop optional+서울 노선위치 모드 getBusPosByRtid). **B** 코스 Phase3(경주 4번째 도시 11스팟+부산+5/제주+4+4페르소나 nightlife/nature/solo/budget; 총 스팟 76·페르소나 10). **C** 템플스테이 큐레이션 프라이머(P3); P4·Jamsil은 D-020에서 이미 처리. **TourAPI(D-027)** 법정동 변경공지 영향평가=영향 없음(areaCode 미사용), 코드 무변경. **254 tests green, 빌드 클린.** ⏳ KC 재배포(중지→시작)+`build` 확인 후 라이브 검증 + 시나리오 테스트.
- [ ] **🔥 다음: ① KC 재배포 + 라이브 검증(서울버스 노선위치 모드·route→track 원탭·경주 코스·템플스테이) ② 시나리오 테스트(사용자 허용요청 후) ③ 추가 Phase: 코스 원클릭 길찾기·영업시간 인라인·다국어 코스, getArrInfoByRoute 정밀화**. (PlayMCP 심사요청=보류, 사용자 결정 대기 — 재촉 금지. 노출키 재발급=사용자 스킵. 비즈폼 사업자번호 png 교체=사용자 액션 대기.) 상세 **docs/25 §5·§6·§10**.
